...
async function seed() {
  try {
    ...
    await sql.begin(async (sql) => {
        ...
        const usersToInsert = [
          { id: fixedAlexDoeId, name: 'Alex Doe', email: 'alex.doe@example.com', password: await bcrypt.hash('password123', 10), role: 'agent' as const, status: 'online' as const, avatar: 'https://picsum.photos/seed/1/40/40' },
        ...
        const casesToInsert = [
            { id: 'c15c8e14-d4b3-4a11-827d-949f5b5b0b8c', customer_id: customerIds.jamie, status: 'in-progress' as const, summary: '关于延迟订单 #12345XYZ 的查询。', updated_at: new Date(Date.now() - 1000 * 60 * 1) },
            { id: '725350b5-7724-434e-b5f7-628d011c7694', customer_id: customerIds.pat, status: 'open' as const, summary: '未指明商品的退货请求。', updated_at: new Date(Date.now() - 1000 * 60 * 118) },
            { id: 'f8c3de3d-1fea-4d7c-a8b0-29f63d6a2f47', customer_id: customerIds.chris, status: 'resolved' as const, summary: '优惠码应用问题。', updated_at: new Date(Date.now() - 1000 * 60 * 60 * 23) },
            { id: '1b9d6bcd-bbfd-4b2d-9b5d-ab8dfbbd4bed', customer_id: customerIds.taylor, status: 'open' as const, summary: '关于国际运输的问题。', updated_at: new Date(Date.now() - 1000 * 60 * 5) }
        ];
        ...
        const messagesToInsert = [
            ...
            { case_id: caseIds.jamie, sender_type: 'user' as const, content: '你好，我最近的订单遇到了问题。还没有送达。', timestamp: new Date(Date.now() - 1000 * 60 * 25), customer_id: customerIds.jamie, user_id: null },
            { case_id: caseIds.jamie, sender_type: 'agent' as const, content: '你好 Jamie，听到这个消息我很难过。您能提供您的订单号吗？', timestamp: new Date(Date.now() - 1000 * 60 * 23), user_id: userIds.alex, customer_id: null },
            { case_id: caseIds.jamie, sender_type: 'user' as const, content: '当然，是 #12345XYZ。', timestamp: new Date(Date.now() - 1000 * 60 * 22), customer_id: customerIds.jamie, user_id: null },
            { case_id: caseIds.jamie, sender_type: 'agent' as const, content: '谢谢你。让我为你查询一下状态。', timestamp: new Date(Date.now() - 1000 * 60 * 21), user_id: userIds.alex, customer_id: null },
            { case_id: caseIds.jamie, sender_type: 'user' as const, content: '好的，我等着。', timestamp: new Date(Date.now() - 1000 * 60 * 1), customer_id: customerIds.jamie, user_id: null },

            // Pat's conversation
            { case_id: caseIds.pat, sender_type: 'user' as const, content: '我想退货。', timestamp: new Date(Date.now() - 1000 * 60 * 120), customer_id: customerIds.pat, user_id: null },
            { case_id: caseIds.pat, sender_type: 'agent' as const, content: '我可以帮忙。您想退货的商品是什么？', timestamp: new Date(Date.now() - 1000 * 60 * 118), user_id: userIds.sam, customer_id: null },
            
            // Chris's conversation
            { case_id: caseIds.chris, sender_type: 'user' as const, content: '我的优惠码无效。', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 24), customer_id: customerIds.chris, user_id: null },
            { case_id: caseIds.chris, sender_type: 'system' as const, content: '此对话已由 Alex Doe 解决。', timestamp: new Date(Date.now() - 1000 * 60 * 60 * 23), user_id: null, customer_id: null },

            // Taylor's conversation
            { case_id: caseIds.taylor, sender_type: 'user' as const, content: '你们向加拿大发货吗？', timestamp: new Date(Date.now() - 1000 * 60 * 5), customer_id: customerIds.taylor, user_id: null },
        ];
